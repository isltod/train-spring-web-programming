...
<html>
  ...
  <body>
      ...
      <c:when test="${fn:length(order.items) != 0}">
        ...
        <table>
          ...
          <c:forEach var="item" items="${order.items}" varStatus="status">
            <tr>
              <form action="/order/remove_from_cart" method="post">
    ...
    <form method="POST" action="/order/place_order">
      ...
      <input type="submit" value="취소" onclick="setActionValue('/order/cancel_order')">
    </form>
    <c:import url="/footer.jsp"/>
    <script>
        function setActionValue(actionString) {
            document.forms[document.forms.length - 1].setAttribute("action", actionString);
        }
    </script>
  </body>
</html>
